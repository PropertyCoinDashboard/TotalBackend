...
class UpBitCandlingAPI(ApiBasicArchitecture):
    ...
    def upbit_candle_day_custom_price(self) -> list:
        return header_to_json(f"{UPBIT_API_URL}/candles/days?{self.name_candle_count_date}")
...
def transform_coinone_schema(data: pd.DataFrame, coin_name: str) -> pd.DataFrame:
    transformed_data: pd.DataFrame = data.rename(columns={
        "open": "opening_price",
        "high": "high_price",
        "low": "low_price",
        "close": "trade_price",
...
def bithum_trade_all_list(coin_name: Optional[str] = None) -> pd.DataFrame:
    bithum_init: list = BithumCandlingAPI(name=coin_name).bithum_candle_price(mint="24h")
